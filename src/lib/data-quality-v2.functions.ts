...
const deliveryLineSql = `
  with raw as (
    ...
    from parsed p
    left join items by_id on p.raw_item_id ~ '^\\d+$' and by_id.id = p.raw_item_id::int and coalesce(by_id.active,true)=true and by_id.deleted_at is null and coalesce(by_id.org_buy_enabled, true) = true
    left join items by_name on by_id.id is null and ${normSql("by_name.name")} = ${normSql("p.raw_item_name")} and coalesce(by_name.active,true)=true and by_name.deleted_at is null and coalesce(by_name.org_buy_enabled, true) = true
  )`;
...
export const getDataQualityReport = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<DataQualityReport> => {
    ...
    checks.push(await check({ id: "items_invalid_price_or_side", area: "precos", severity: "high", title: "Itens ativos com side/preços inválidos", countSql: `select count(*)::int as count from items where ${ACTIVE_ITEM} and (coalesce(side, '') not in ('venda','compra','ambos') or (side in ('venda','ambos') and ${SALE_PRICE} <= 0) or (side in ('compra','ambos') and coalesce(org_buy_enabled, true) = true and ${BUY_PRICE} <= 0))`, examplesSql: `select ('#' || id::text || ' · ' || name || ' · side=' || coalesce(side, '—') || ' · civil=' || coalesce(purchase_price::text, '—') || ' · org=' || coalesce(morador_purchase_price::text, '—') || ' · compra_org=' || coalesce(org_buy_enabled::text, 'true') || ' · definido=' || coalesce(min_sale_price::text, '—')) as label from items where ${ACTIVE_ITEM} and (coalesce(side, '') not in ('venda','compra','ambos') or (side in ('venda','ambos') and ${SALE_PRICE} <= 0) or (side in ('compra','ambos') and coalesce(org_buy_enabled, true) = true and ${BUY_PRICE} <= 0)) order by category, name limit 10`, summary: (n) => n ? `${n} item(ns) ativo(s) com side ou preço inválido.` : "Itens ativos têm side/preços mínimos coerentes.", recommendation: "Para venda basta preço com material OU sem material; para compra basta preço civil OU organização/definido quando a compra pela org estiver ativa." }));
