...
export const listOrders = createServerFn({ method: "GET" })
  ...
  .inputValidator((d: { scope?: "mine" | "manage"; status?: string | null }) => ({
    scope: d?.scope ?? "mine",
...
export const createOrder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { item_id: number; quantity: number; notes?: string | null }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    const item = await pgOne<{ name: string; side: string | null; base: number | null }>(
      `select name, side, min_sale_price::float as base from items where id = $1 and active = true`,
      [data.item_id]
    );
    if (!item) throw new Error("Item não encontrado");
    if (item.side !== "venda") throw new Error("Esse item não está disponível para encomenda");
    const margin = tierMargin(me.tier);
    const unit = item.base != null ? Math.round(item.base * (1 + margin)) : null;
    ...
    const row = await pgOne<{ id: number }>(
      `insert into orders
       ...
       returning id`,
      [me.id, data.item_id, data.quantity, unit, total, data.notes ?? null, margin * 100, `web:${context.userId}`]
    );
...
export const transitionOrder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: number; to: OrderStatus; notes?: string | null }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    await pgQuery(
      ...
      [data.id, before.status, data.to, `web:${context.userId}`, data.notes ?? null]
    );
    // notify requester
