...
function filterOrderIngredients(output: OutputCategory, ingredients: RecipeIngredient[]): RecipeIngredient[] {
  return isOrangeCategory(output) ? ingredients.filter(isPiecesIngredient) : ingredients;
}
...
async function assertResponsibleExists(memberId: number) {
  ...
  if (!responsible) {
    throw new Error("Responsável inválido. Só Patrão di Zona, Kingpin ou Manda-Chuva podem ser responsáveis.");
  }
}
...
async function getOrderIngredients(
  ...
  const dbRows = await pgQuery<{ name: string; qty: number; category: string | null; subcategory: string | null }>(
    `select i.name,
      ...
      where cr.item_id = $1
        ...
        and coalesce(coalesce(o.quantity, ri.quantity), 0) > 0
      order by i.name`,
    [itemId, tier],
  ).catch(() =>
    pgQuery<{ name: string; qty: number; category: string | null; subcategory: string | null }>(
      `select i.name,
              i.category,
              i.subcategory,
  ...
  return filterOrderIngredients(
    output,
    dbRows.map((row) => ({
      name: row.name,
      category: row.category,
      subcategory: row.subcategory,
      needed: Number(row.qty) * quantity,
    })),
...
async function withCurrentOrderValues(rows: OrderRow[]): Promise<OrderRow[]> {
  return Promise.all(
    rows.map(async (row) => {
      ...
      const priceTier = resolveMemberPriceTier(row.member_tier ?? null, row.member_role ?? null);
      const itemSurcharges = await getSurchargeForItem(row.item_id).catch(() => new Map());
      const prices = resolveItemPrices(dbItem, null, priceTier, itemSurcharges);
      const currentIngredients = await getOrderIngredients(row.item_id, dbItem, row.quantity, priceTier).catch(
  ...
  .handler(async ({ data, context }): Promise<OrderRow[]> => {
    ...
    const rows = await pgQuery<OrderRow>(
      `select o.id,
              ...
              coalesce(m.role,'bairrista') as member_role,
              o.item_id,
              i.name as item_name,
              i.category as item_category,
              i.subcategory as item_subcategory,
              i.side as item_side,
              i.min_sale_price::float as item_min_sale_price,
              i.purchase_price::float as item_purchase_price,
              i.morador_purchase_price::float as item_morador_purchase_price,
              i.estimated_value::float as item_estimated_value,
              o.quantity,
              o.status,
...
export const createOrder = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (d: {
      ...
      notes?: string | null;
      responsavel_member_id?: number | null;
      payment_mode?: "materials_money" | "money_only";
    }) => {
      ...
      for (const line of d.lines) {
        if (!Number.isFinite(line.item_id) || line.item_id <= 0) throw new Error("Item inválido");
        if (!Number.isFinite(line.quantity) || line.quantity <= 0) throw new Error("Quantidade inválida");
      }
      const responsavelMemberId = d.responsavel_member_id;
      if (responsavelMemberId == null || !Number.isFinite(responsavelMemberId) || responsavelMemberId <= 0) {
      ...
      return {
        ...d,
        responsavel_member_id: responsavelMemberId,
        payment_mode: paymentMode,
        notes: NotesSchema.parse(d.notes),
      };
    },
  )
  .handler(async ({ data, context }) => {
    ...
    for (const line of data.lines) {
      ...
      const itemPrices = resolveItemPrices(dbItem, null, priceTier, itemSurcharges);
      ...
      const ingredients = await getOrderIngredients(dbItem.id, dbItem, line.quantity, priceTier);
      ...
      const row = await pgOne<{ id: number }>(
        `insert into orders
          (member_id, item_id, quantity, status, unit_price, total_price, notes, markup_percent,
           created_at, updated_at, updated_by, responsavel_member_id, ingredients_json, batch_id,
           payment_mode, material_cost, money_cost)
    ...
    void logAdminAction(context.supabase, {
      ...
      targetType: "order",
      targetId: results.map((result) => result.id).join(","),
      details: `${results.length} encomenda(s) criada(s)`,
...
export const transitionOrder = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { id: number; to: OrderStatus }) => ({
    id: IdSchema.parse(d.id),
    to: OrderStatusSchema.parse(d.to) as OrderStatus,
  }))
  .handler(async ({ data, context }) => {
    const me = await resolveCurrentMember(context.supabase, context.userId);
...
export const cancelOwnOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await Promise.all(
      data.ids.map((id) => insertOrderHistory(id, "pending", "cancelled", `web:${context.userId}`, "Cancelada pelo membro")),
    );
...
export const listOrderComments = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { order_id: number }) => ({ order_id: IdSchema.parse(d.order_id) }))
  .handler(async ({ data }) =>
    pgQuery<OrderCommentRow>(
      `select c.id,
              c.content,
              c.created_at,
              coalesce(c.author_name, m.display_name, m.nickname) as author_name
...
export const addOrderComment = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { order_id: number; content: string }) => ({
    order_id: IdSchema.parse(d.order_id),
    content: z.string().trim().min(1).max(1000).parse(d.content),
  }))
  .handler(async ({ data, context }) => {
    const me = await resolveCurrentMember(context.supabase, context.userId);
    if (!me) throw new Error("Membro não encontrado");
