...
import { listUnfinalizedSaidas, getSaidaDetail, liquidateSaida } from "@/lib/liquidation.functions";
        ...
        {(list.data ?? []).map((s) => (
          <div key={s.id} className="flex items-center gap-4 rounded-sm border border-border bg-card p-4">
            ...
            <Button size="sm" variant="outline" onClick={() => setOpenId(s.id)}>
              <FileSearch className="mr-1 h-4 w-4" />Liquidar
            </Button>
          </div>
        ))}
        {!list.isLoading && !list.data?.length && <p className="text-muted-foreground">Nenhuma saída pendente.</p>}
      </div>

      <Dialog open={openId != null} onOpenChange={(v) => !v && setOpenId(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>Saída #{openId}</DialogTitle></DialogHeader>
          {detail.isLoading && <p className="text-muted-foreground">A carregar…</p>}
          {detail.data && (
            <div className="space-y-4 text-sm">
                ...
                <div className="max-h-48 overflow-auto rounded-sm border border-border">
                  <table className="w-full text-xs">
                    <thead className="bg-muted/50 text-muted-foreground"><tr>
                      <th className="px-2 py-1 text-left">Membro</th><th className="px-2 py-1">K/D</th><th className="px-2 py-1">Net</th><th className="px-2 py-1">Settled</th>
                    </tr></thead>
                    <tbody>
              ...
              <div>
                <div className="text-display text-xs text-muted-foreground mb-1">Materiais ({detail.data.materials.length})</div>
                <div className="max-h-32 overflow-auto rounded-sm border border-border text-xs">
                  ...
                  {!detail.data.materials.length && <div className="px-2 py-2 text-muted-foreground">Sem materiais registados.</div>}
                </div>
              </div>
            </div>
          )}
          <DialogFooter>
            ...
            <Button onClick={() => liq.mutate(openId!)} disabled={liq.isPending || !detail.data}>
              <CheckCircle2 className="mr-1 h-4 w-4" />{liq.isPending ? "A liquidar…" : "Liquidar agora"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
