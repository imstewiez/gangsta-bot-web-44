  ...
  return (
    <>
      <PageHeader
        ...
        description={`${data?.length ?? 0} registos.`}
        action={<NewSaida />}
      />
      <div className="grid gap-3">
        ...
        {(data ?? []).map((s) => (
          ...
          >
            <div className="text-display text-2xl text-primary w-12 text-center">
              #{s.id}
            </div>
            ...
            <span
              className={
                "rounded-sm px-2 py-1 text-display text-xs " +
                (STATUS_COLOR[s.status] ?? "bg-muted")
              }
            >
              {s.status}
            </span>
        ...
        {!isLoading && !data?.length && (
          <p className="text-muted-foreground">Sem operações.</p>
        )}
      </div>
    </>
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button disabled={!type || m.isPending} onClick={() => m.mutate()}>
