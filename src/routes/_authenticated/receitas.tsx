...
function Page() {
  ...
  const calc = useMutation({
    mutationFn: () => calcFn({ data: { recipe_id: calcRecipe!, quantity: qty } }),
  ...
  return (
    <>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {!recipes.isLoading && !filtered.length && <p className="text-muted-foreground">Sem receitas.</p>}
      </div>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => calc.mutate()} disabled={calc.isPending}>
              <Hammer className="mr-1 h-4 w-4" />{calc.isPending ? "…" : "Calcular"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
