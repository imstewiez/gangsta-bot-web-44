...
import { Banknote, Loader2, MessageSquare, Package, Plus, Send, ShoppingBag, Trash2, X } from "lucide-react";
...
function OrdersList({
  ...
  const transitionM = useMutation({
    mutationFn: (v: { id: number; to: string }) =>
      transFn({ data: { id: v.id, to: v.to as OrderStatus } }),
    onSuccess: () => {
    ...
    return (
      <Card className="interactive-card p-10 text-center">
        ...
        <p className="text-sm font-medium text-foreground">
          {statusFilter === "active" ? EMPTY_STATE.orders.title : EMPTY_STATE.ordersHistory.title}
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          {statusFilter === "active" ? EMPTY_STATE.orders.description : EMPTY_STATE.ordersHistory.description}
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="grid gap-3">
      {batches.map(([batchId, lines]) => {
        ...
        const totalDirtyMoney = lines.reduce((sum, line) => sum + (line.dirty_money ?? 0), 0);
            ...
            <div className="flex flex-wrap items-start gap-4">
              <div className="min-w-[200px] flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={cn("rounded-sm border px-2 py-0.5 text-display text-[10px] uppercase tracking-wider", STATUS_COLOR[first.status])}>
                ...
                <div className="mt-1.5 text-sm text-muted-foreground">
                  ...
                  {first.responsavel_name && (
                    <span className="mt-0.5 block">
                      Responsável: <span className="font-medium text-foreground">{first.responsavel_name}</span>
                    </span>
                  )}
                ...
                <div className={cn("mt-2 space-y-1.5 rounded-sm border p-2 text-xs", hasMaterials ? "border-border bg-muted/30" : "border-amber-500/20 bg-amber-500/5")}>
                  {hasMaterials ? (
                    <>
                      ...
                      <ul className="space-y-0.5">
                        {aggregatedIngredients.map((ingredient) => (
                          <li key={ingredient.name} className="interactive-row flex items-center justify-between">
                            <span className="text-foreground">{ingredient.name}</span>
                            <span className="font-mono text-muted-foreground">{fmtNum(ingredient.needed)}</span>
                          </li>
                        ))}
                      </ul>
                      {totalDirtyMoney > 0 && (
                        <div className="flex items-center justify-between border-t border-border pt-1.5 font-semibold">
                          ...
                          <span className="font-mono text-emerald-400">{fmtPrice(Math.round(totalDirtyMoney))}</span>
                        </div>
                      )}
                    </>
                  ) : (
                    <div className="flex items-center gap-1.5 text-amber-400">
                      ...
                      <span className="font-semibold">Pagamento só em dinheiro</span>
              ...
              <div className="text-right">
                <div className="font-mono text-lg font-semibold">{fmtPrice(totalBatch)}</div>
              ...
              {(next || canCancel) && (
                <div className="flex w-full justify-end gap-1.5 border-t border-border pt-3">
                  {canCancel && (
                    <ButtonLoading size="sm" variant="outline" loading={cancelM.isPending} onClick={handleCancel} className="text-destructive hover:text-destructive">
                  ...
                  {next?.map((state) => (
                    <ButtonLoading
                      key={state.to}
                      size="sm"
                      variant={state.variant === "destructive" ? "outline" : "default"}
                      loading={transitionM.isPending}
                      onClick={() => handleTransition(state.to)}
                    >
                      {state.label}
                    </ButtonLoading>
                  ))}
...
function OrderCommentThread({ orderId, canComment }: { orderId: number; canComment: boolean }) {
  ...
  return (
    <div className="mt-3 border-t border-border pt-2">
      <button onClick={() => setOpen((value) => !value)} className="flex items-center gap-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground">
        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="max-h-64 space-y-2 overflow-y-auto">
            {(comments.data ?? []).map((comment) => (
              <div key={comment.id} className="rounded-sm bg-muted/40 p-2 text-sm">
            ...
            {!comments.isLoading && !(comments.data ?? []).length && (
              <p className="py-4 text-center text-xs text-muted-foreground">Sem comentários.</p>
            )}
          </div>
          {canComment && (
            <div className="flex gap-2">
              ...
              />
              <ButtonLoading size="sm" loading={addM.isPending} disabled={!text.trim()} onClick={() => addM.mutate(text.trim())} className="h-8 px-2">
...
function NewOrder() {
  ...
  const items = useMemo(() => {
    return (catalog.data ?? []).filter((item: CatalogItem) => {
  ...
  const sim = useQuery({
    ...
    queryFn: () =>
      simFn({
        data: {
          lines: validLines.map((line) => ({ item_id: Number(line.item_id), quantity: lineQuantity(line) })),
        },
      }),
  ...
  useEffect(() => {
    if (sim.data && sim.data.ingredients.length === 0 && paymentMode !== "money_only") {
      setPaymentMode("money_only");
    }
    ...
    for (const category of ARMORY_CAT_ORDER) {
      ...
      result.push(
        ...list.map((item) => ({
          ...
          label: item.name,
          group: cfg?.label ?? fmtCategoryLabel(category),
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (value ? setOpen(true) : handleClose())}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{step === "select" ? "O que precisas?" : "Confirma a tua encomenda"}</DialogTitle>
        </DialogHeader>
            ...
            <div className="grid max-h-[60vh] gap-3 overflow-y-auto pr-1">
              {lines.map((line, index) => (
                <div key={index} className="grid grid-cols-[1fr_100px_auto] gap-2">
                  ...
                  />
                  <Input type="number" min={1} value={line.qty} onChange={(event) => updateLine(index, { qty: event.target.value })} />
                  <Button size="sm" variant="ghost" onClick={() => setLines((current) => current.filter((_, i) => i !== index))} disabled={lines.length === 1}>
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
              ))}

              <Button size="sm" variant="outline" onClick={() => setLines((current) => [...current, { item_id: "", qty: "1" }])}>
                <Plus className="mr-1 h-4 w-4" /> Mais uma linha
              </Button>

              {validLines.length > 0 && !sim.isLoading && (
                <div>
                  <label className="mb-1 block text-xs text-muted-foreground">Modo de encomenda</label>
                  {hasMaterialQuote ? (
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={cn(
                          ...
                          paymentMode === "materials_money" ? "border-primary bg-primary/10" : "border-border bg-muted/30 hover:bg-muted/50",
                        )}
                      >
                        <Package className={cn("mb-1.5 h-5 w-5", paymentMode === "materials_money" ? "text-primary" : "text-muted-foreground")} />
                        <div className={cn("text-xs font-semibold", paymentMode === "materials_money" ? "text-primary" : "text-foreground")}>Com materiais</div>
                        <div className="mt-0.5 text-[10px] leading-tight text-muted-foreground">Entrego materiais necessários + dinheiro</div>
                      </button>
                      <button
                        type="button"
                        ...
                        className={cn(
                          "rounded-lg border p-3 text-left transition-colors",
                          paymentMode === "money_only" ? "border-primary bg-primary/10" : "border-border bg-muted/30 hover:bg-muted/50",
                        )}
                      >
                        <Banknote className={cn("mb-1.5 h-5 w-5", paymentMode === "money_only" ? "text-primary" : "text-muted-foreground")} />
                        <div className={cn("text-xs font-semibold", paymentMode === "money_only" ? "text-primary" : "text-foreground")}>Sem materiais</div>
                        <div className="mt-0.5 text-[10px] leading-tight text-muted-foreground">Pago material em dinheiro + taxa</div>
                      </button>
                    </div>
                  ) : (
                    <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-400">
                      ...
                      <p className="mt-1 text-[10px] text-muted-foreground">
                        Os artigos selecionados não têm receita/material associado, por isso não existe opção de entregar materiais.
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <SelectContent>
                    {(managers.data ?? []).map((manager) => (
                      <SelectItem key={manager.id} value={String(manager.id)}>
                        {manager.display_name ?? manager.nick ?? `Membro #${manager.id}`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                ...
                <Textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={2} placeholder="Notas" />
              </div>
            </div>

            <DialogFooter>
              <Button variant="ghost" onClick={handleClose}>Deixa lá</Button>
              <Button disabled={!canReview} onClick={() => setStep("checkout")}>Rever encomenda →</Button>
            </DialogFooter>
          </>
        ) : (
          <>
            <div className="grid max-h-[60vh] gap-3 overflow-y-auto pr-1">
              <div className={cn("flex items-center gap-2 rounded-sm border p-2 text-xs", effectivePaymentMode === "money_only" ? "border-amber-500/30 bg-amber-500/10 text-amber-400" : "border-blue-500/30 bg-blue-500/10 text-blue-400")}>
                {effectivePaymentMode === "money_only" ? <Banknote className="h-3.5 w-3.5" /> : <Package className="h-3.5 w-3.5" />}
                <span className="font-medium">{effectivePaymentMode === "money_only" ? "Só dinheiro (sem materiais)" : "Materiais + dinheiro"}</span>
              </div>

              <div>
                <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Itens</div>
                <ul className="space-y-0.5 text-sm">
                  {validLines.map((line, index) => {
                    const item = itemsById.get(line.item_id);
                    const qty = lineQuantity(line);
                    return (
                      <li key={index} className="interactive-row flex items-center justify-between">
                        <span className="font-medium">{qty}× {item?.name ?? "—"}</span>
                        <span className="font-mono text-xs text-muted-foreground">{fmtPrice(itemUnitPrice(item) * qty)}</span>
                      </li>
                    );
                  })}
                </ul>
              ...
              <div className="rounded-sm border border-border bg-muted/30 p-3">
                <div className="flex items-center justify-between text-sm font-semibold">
                  ...
                  <span className="font-mono">{fmtPrice(Math.round(finalTotal))}</span>
                </div>
                {effectivePaymentMode === "money_only" && hasMaterialQuote && (
                  <div className="mt-1 space-y-0.5 text-[10px] text-muted-foreground">
                    <div className="flex justify-between"><span>Base:</span><span className="font-mono">{fmtPrice(Math.round(baseTotal))}</span></div>
                    <div className="flex justify-between"><span>Material:</span><span className="font-mono">{fmtPrice(materialCost)}</span></div>
                    <div className="flex justify-between"><span>Taxa (20%):</span><span className="font-mono">{fmtPrice(moneyOnlyMarkup)}</span></div>
                  </div>
                )}
              </div>

              {effectivePaymentMode === "materials_money" && hasMaterialQuote && (
                <div>
                  <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Materiais a entregar</div>
                  <ul className="space-y-0.5 text-xs">
                    {(sim.data?.ingredients ?? []).map((ingredient) => (
                      <li key={ingredient.name} className="interactive-row flex items-center justify-between">
              ...
              {!hasMaterialQuote && (
                <div className="rounded-sm border border-amber-500/20 bg-amber-500/5 p-2 text-xs text-muted-foreground">
                  Esta encomenda não requer materiais. Vais pagar apenas o preço definido do artigo.
                </div>
              )}
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-foreground">{selectedManager?.display_name ?? selectedManager?.nick ?? "—"}</span>
                </div>
                {notes && (
                  <div className="flex justify-between gap-3">
                    <span>Notas:</span>
                    <span className="text-right italic">&quot;{notes}&quot;</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <DialogFooter>
              ...
              <ButtonLoading loading={createM.isPending} disabled={createM.isPending} onClick={() => createM.mutate()}>
                {createM.isPending ? "A processar" : "Confirmar encomenda"}
              </ButtonLoading>
            </DialogFooter>
          </>
        )}
