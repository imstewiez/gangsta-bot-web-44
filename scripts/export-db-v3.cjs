...
function escapeCopy(s) {
  // For COPY TEXT format: escape backslash, tab, newline, carriage return
  return s.replace(/\\/g, "\\\\").replace(/\t/g, "\\t").replace(/\n/g, "\\n").replace(/\r/g, "\\r");
}

function formatValue(v, pgType) {
  if (v === null || v === undefined) return "\\N";

  const isJson = pgType && (pgType.includes("json") || pgType.includes("JSON"));

  // Handle Date objects
  if (v instanceof Date) {
    if (isJson) {
      // JSON needs quoted ISO string
      return escapeCopy(JSON.stringify(v.toISOString()));
    }
    return escapeCopy(v.toISOString());
  }
  ...
  if (Array.isArray(v)) {
    if (isJson) {
      // json/jsonb column: serialize as JSON array/object
      return escapeCopy(JSON.stringify(v));
    }
    // PostgreSQL array literal format
    // Each element needs to be quoted if string, or formatted if other type
    const elements = v.map((item) => {
      if (item === null) return "NULL";
      if (typeof item === "boolean") return item ? "t" : "f";
      if (typeof item === "number") return String(item);
      if (item instanceof Date) return `"${escapeCopy(item.toISOString())}"`;
      // String items need quoting and escaping
      return `"${escapeCopy(String(item))}"`;
    });
    return "{" + elements.join(",") + "}";
  }

  // Handle objects (including plain objects, Buffer, etc.)
  if (typeof v === "object") {
    if (isJson) {
      return escapeCopy(JSON.stringify(v));
    }
    // For non-JSON columns with objects, try JSON.stringify as fallback
    return escapeCopy(JSON.stringify(v));
  }

  // Default: string
  return escapeCopy(String(v));
}
...
async function exportDb() {
  ...
  let fullSql = "-- Exported from Railway PostgreSQL (v3)\n";
  fullSql += "BEGIN;\n\n";
  fullSql += "SET CONSTRAINTS ALL DEFERRED;\n\n";
  ...
  for (const { table_name } of tables) {
    ...
    if (pks.length) fullSql += "\n";
  }

  // === UNIQUE CONSTRAINTS ===
  console.log("Exporting unique constraints...");
  for (const { table_name } of tables) {
  ...
  for (const { table_name } of tables) {
    ...
    `;
    for (const fk of fks) {
      // Make FK deferrable so data can be loaded in any order
      let def = fk.def;
      if (!def.includes("DEFERRABLE")) {
        def += " DEFERRABLE INITIALLY DEFERRED";
      }
      fullSql += `ALTER TABLE "${table_name}" ADD CONSTRAINT "${fk.conname}" ${def};\n`;
    }
    if (fks.length) fullSql += "\n";
  }

  // === CHECK CONSTRAINTS ===
  console.log("Exporting check constraints...");
  for (const { table_name } of tables) {
  ...
  for (const { table_name } of tables) {
    ...
    for (let offset = 0; offset < count; offset += batchSize) {
      ...
      for (const row of Array.from(rows)) {
        const line = cols.map((c) => formatValue(row[c.column_name], c.data_type)).join("\t");
        fullSql += line + "\n";
      }
    }
    fullSql += "\\.\n\n";
  ...
  fs.writeFileSync(path.join(outDir, "dump-v3.sql"), fullSql);
  console.log(`\nDone! Exported to ${path.join(outDir, "dump-v3.sql")}`);
  console.log(`Size: ${(fs.statSync(path.join(outDir, "dump-v3.sql")).size / 1024 / 1024).toFixed(1)} MB`);
  await sql.end();
}
